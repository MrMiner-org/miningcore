...
- Hybrid PoW/PoS coins supported via the `isPseudoPoS` coin property. Mining
  targets the PoW chain while staking rewards accrue on the PoS chain
...
`"isPseudoPoS": true`. Mining then occurs on the coin's PoW chain while your
wallet continues to stake on the PoS chain.
